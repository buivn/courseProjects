...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250, 
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            D_solver.zero_grad()
            G_solver.zero_grad()
            true_discriminator_out = D(real_images)
            fake_discriminator_out = D(fake_images)
            d_error = discriminator_loss(true_discriminator_out, fake_discriminator_out)
            d_error.backward()
            D_solver.step()
            ...
            G_solver.zero_grad()
            fake_discriminator_out = D(fake_images)
            g_error = generator_loss(fake_discriminator_out)
            g_error.backward()
            G_solver.step()
