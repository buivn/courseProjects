...
class Perceptron:
  ...
  def train(self, X_train: np.ndarray, y_train: np.ndarray):       
    ...
    check = y_train.reshape(len(y_train),1)
    onehot_encoded = onehot_en.fit_transform(check)
    # convertt he one hot encoder into sign encoder
    sign_encoded = np.where(onehot_encoded == 0.0, -1.0, onehot_encoded)
    ...
    for j in range(self.epochs):
      for i in range(len(X_train)):
        ...
        if np.argmax(outputs) == np.argmax(sign_encoded[i]):
          pass
        else:
          # if the number of incorrect prediction is more, update the weights:
          cor_index = np.argmax(sign_encoded[i])
          # correct_indices = np.argwhere(results >= 0.0)
          for k in range(self.n_class):
            if outputs[k] > outputs[cor_index]:
