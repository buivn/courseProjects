...
class Softmax:
  ...
  def calc_gradient(self, X_train: np.ndarray, y_train: np.ndarray) -> np.ndarray:
    ...
    for i in range(len(X_train)):
      ...
      max_value = np.amax(outputs)
      outputs_1 = np.copy(outputs)
      # besure the exponential is not to high
      for k in range(len(outputs_1)):
        outputs_1[k] -= max_value
        outputs_1[k] = math.exp(outputs_1[k])

      for j in range(self.n_class):
        # if the number of incorrect prediction is more, update the weights:
        sum_all = np.sum(outputs_1)
        if j==correct_index:
          P = outputs_1[correct_index]/sum_all
          delta_w[correct_index] += self.lr*(1-P)*data
        else:  
          P = outputs_1[j]/sum_all
          delta_w[j] -= self.lr*P*data  
  ...
  def train(self, X_train: np.ndarray, y_train: np.ndarray):
    ...
    for j in range(self.epochs):
      ...
      for i in range(numOfRun):
        ...
        self.w -=  self.lr*self.reg_const*self.w/49000.0
...
if __name__ == '__main__':
  ...
  softmax = Softmax(n_class, lr, epochs, reg_const) 
  softmax.train(f_X_train, y_train)
  results = softmax.predict(f_X_train)
  res = y_train - results  
  correct = len(np.argwhere(res == 0))/(len(y_train))
  print("The accuracy of X_train: ", correct)

  results = softmax.predict(f_X_val)
  res = y_val - results  
  correct = len(np.argwhere(res == 0))/(len(y_val))
  print("The accuracy of X_eval: ", correct)


  results = softmax.predict(f_X_test)
  res = y_test - results  
  correct = len(np.argwhere(res == 0))/(len(y_test))
  print("The accuracy of X_test: ", correct)

  output_path = '/home/dzungbui/Dropbox/phd_gmu/cs747_deepLearning/assignments/CS747-assignment1/models/softmax_outputs.csv'
