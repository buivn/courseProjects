...
;; Basic Operations
;; &rest as you want to input a list
...
;; This function calculate the error between two input column-vectors
(defun net-error (output correct-output)
  ...
  (let ((sum 0.0) diff)
    (dotimes (i (length output))
      (setf diff (- (first (elt output i)) (first (elt correct-output i))))
...
the datum as input."
  (let (o2)
    (setf o2 (sigmoid-a-2dlist (multiply v datum)))
    (sigmoid-a-2dlist (multiply w o2))))

(defun sigmoid-a-2dlist (list)
  ;; This function return a vector of result from sigmoid function for a 2d list
  (let (result (result-list nil) rl)
    (dolist (item list)
      (setf result (sigmoid (first item)))
      (setf rl (reverse result-list))
      (if (null result-list)
	 (setf result-list (list (list result)))
	 (setf result-list (reverse (push (list result) rl)))))
    result-list))

(defun sum-matrix-column (matrix)
  ;; This function sum all the element in the same column and return a vector of them
  (let ((output-list nil) rl (sum 0))
    (dotimes (j (length (first matrix)))
      (dolist (row  matrix)
	(setf sum (+ sum (elt row j))))
      
      (setf rl (reverse output-list))
      (if (null output-list)
	  (setf output-list (list (list sum)))
	  (setf output-list (reverse (push (list sum) rl))))
      (setf sum 0.0))
    output-list))
      

(defun back-propagate (datum1 alpha v w)
  "Back-propagates a datum through the V and W matrices,
returning a list consisting of new, modified V and W matrices."
  ;; This function works with only one datum item
  (let* ((o nil) (h nil) (sig-list-h nil) (sig-list-o nil) (delta-w nil) (delta-v nil)  term1 term2 (w-delta-w nil) (label nil) (datum nil))
    ;; split the data and label
    (setf datum (first datum1))
    (setf label (first (rest datum1)))
    
    ;; calculate the value at hidden layer nodes
    (setf h (multiply v datum))
    (setf sig-list-h (sigmoid-a-2dlist h))

    ;; calculat the value of output layer nodes
    (setf o (multiply w sig-list-h))
    (setf sig-list-o (sigmoid-a-2dlist o))
    
    ;; setup the size of matrix delta-w
    (setf delta-w (make-random-matrix (length w) (length (first w)) 0.01))
    ;; calculate the update term of matrix delta-w 
    (dotimes (i (length o))
      (setf term1 (- (first (elt label i)) (first (elt sig-list-o i))))
      ;(print term1)
      (setf term2 (* term1 (- 1 (first (elt sig-list-o i)))))
      (setf term2 (* alpha term2))
      (setf term2 (* term2 (first (elt sig-list-o i))))
      ;; calculate the matrix delta-w 
      (dotimes (j (length h))
	(setf (elt (elt delta-w i) j) (* term2 (first (elt sig-list-h j))))))

    ;;; calculate the sum of all columns in matrix w*delta-w
    (setf w-delta-w (sum-matrix-column (e-multiply w delta-w)))
    
    ;; initialize the matrix delta-v 
    (setf delta-v (make-random-matrix (length v) (length (first v)) 0.01))

    ;;calculate the update matrix delta-v 
    (dotimes (j (length v))
      (setf term2 (- 1 (first (elt sig-list-h j))))
      ; multiply w-delta-w and (1-h)
      (setf term2 (* term2 (first (elt w-delta-w j))))      
      (dotimes (k (length datum))
	(setf (elt (elt delta-v j) k) (* term2 (first (elt datum k))))))
  
    ;; update w and v matrices
    (setf w (subtract w delta-w))
    (setf v (subtract v delta-v))
    (list v w)))

(defun back-propagate1 (datum alpha v w)
  ;; This function works with only one datum item
  (let* ((o nil) (h nil) (delta-w nil) (delta-v nil) (x nil) (y nil) (d nil)  (e nil))
    ;; split the data and label
    (setf x (first datum))
    (setf y (first (rest datum))) 
    ;; calculate the value at hidden layer nodes
    (setf h (sigmoid-a-2dlist (multiply v x)))
    ;; calculat the value of output layer nodes
    (setf o (sigmoid-a-2dlist (multiply w h)))
    ;; calculate the vector d
    (setf d (subtract y o))
    (setf d (e-multiply d o))
    (setf d (e-multiply d (subtract-from-scalar 1 o)))
    ;; calculate the intermediate matrix - delta-w 
    (setf delta-w (multiply d (transpose h)))
    (setf delta-w (scalar-multiply alpha delta-w))
    ;; calcualt the vector e
    (setf e (subtract-from-scalar 1 h))
    (setf e (e-multiply h e))
    (setf e (e-multiply e (multiply (transpose w) d)))
    ;; calculate the intermediate matrix - delta-v
    (setf delta-v (multiply e (transpose x)))
    (setf delta-v (scalar-multiply alpha delta-v))  
    ;; update w and v matrices
    (setf w (add w delta-w))
    (setf v (add v delta-v))
    ; return v and w
    (list v w)))



(defun net-build (data num-hidden-units alpha initial-bounds max-iterations modulo &optional print-all-errors (error-each-epoch t))
  (let (input-size output-size (w nil) (v nil) (vw nil) (worst-error 0) (mean-error 0) error predict data1 (j 0))
    ;; determine the input-size and output-size
    (setf input-size (length (first (first data))))
    (setf output-size (length (first (rest (first data)))))
    ;; initialize the matrices v and w
    (setf v (make-random-matrix num-hidden-units input-size initial-bounds))
    (setf w (make-random-matrix output-size num-hidden-units initial-bounds))
    ;; start training the network 
    (dotimes (i max-iterations)
      ;; shuffle the data
      (setf data1 (shuffle data))
    
      (dolist (item  data1)
	;; just send one datum item to the back-propagate function
	(setf vw (back-propagate1 item alpha v w))
	(setf v (first vw))
	(setf w (first (rest vw)))) ;; finish one epoch
      (setf j (+ j 1))
      (cond ((= j modulo)
	(setf j 0)
	;; run forward-propagate to calculate the error after the current epoch
	(dolist (item data1)
	  (setf predict (forward-propagate (first item) v w))
	  ;(print "The predicted result is: ")
	  ;(print predict)
	  (setf error (net-error predict (first (rest item))))
          (cond (print-all-errors
 	      (format t "The error for each sample iteration ~4D: ~8F ~%" i error)))
	      ;;(format t "The predicion: ~10F  ~%" predict)
	      ;;(format t "The label of the sample: ~15F ~%" (rest item))))
	  (if (< worst-error error)
	    (setf worst-error error))
	  (setf mean-error (+ error mean-error))) ;; end of dolist

	;; calculate the mean error during the training in modulo epoch
	(setf mean-error (/ mean-error (length data)))
	(cond (error-each-epoch     
	         (format t "The worst error of iteration ~3D: ~8F ~%" i worst-error)
	         (format t "The mean error of interation ~4D: ~8F ~%" i mean-error)))
	;; terminate the dotimes list if the mean-error is too small
	(if (< mean-error *a-good-minimum-error*)
	   (return))
	(setf worst-error 0)
	(setf mean-error 0))))
    (list v w)))
...
(defun simple-generalization (data num-hidden-units alpha initial-bounds max-iterations)
  (let ((aver-error 0) 2nn-model processed-data train-data test-data v w error average-error predict)
    ;; pre-processing the data
    (setf processed-data (convert-data data))
    ;; divide the data into two sets: train and test data
    (setf train-data (butlast processed-data (round (* 0.5 (length processed-data)))))
    (setf test-data (last processed-data (round (* 0.5 (length processed-data)))))
    ;; build a model and train
    (setf 2nn-model (net-build train-data num-hidden-units alpha initial-bounds max-iterations 200 nil nil))
    (setf v (first 2nn-model))
    (setf w (first (last 2nn-model)))
    ;; run forward-propagate to calculate the error
    (dolist (item test-data)
      (setf predict (forward-propagate (first item) v w))
      (setf error (net-error predict (first (rest item))))
      (setf aver-error (+ error aver-error)))
    ;; calculate the average error
    (setf aver-error (/ aver-error (length test-data)))
    (format t "The average error on test data: ~6F" aver-error)
    average-error))

(defun split (list n k)
  ;; this function divides the input list into two,
  ;; one of them contains the elements from position n to k of the original one 
  (let ((sub-list1 nil) (sub-list2 nil))
    (setf sub-list1 (subseq list n k))  ;; get the list consists of elements n to k
    ;; the second list consists of the elements left
    (setf sub-list2 (reduce #'cons (subseq list 0 n) :initial-value (nthcdr k list) :from-end t))
    (list sub-list1 sub-list2)))

	    
(defun k-fold-validation (data k num-hidden-units alpha initial-bounds max-iterations)
  (let ((aver-error 0) 2nn-model item-number (train-data nil) (test-data nil) data1 (total-error-aver 0) v w predict error)
    (setf item-number (floor (/ (length data) k)))
    (dotimes (i k)   ;; run for each fold
      (cond ((= i (- k 1)) ;; if it is the last fold
	     (setf train-data (subseq data 0 (* item-number i)))
	     (setf test-data (nthcdr (* item-number i) data)))
	  ;; for other folds - use the split function
	  (t (setf data1 (split data (* i item-number) (* (+ i 1) item-number)))
	   (setf test-data  (first data1))
	   (setf train-data (first (rest data1)))))
      
      ;; pre-processing the data
      (setf train-data (convert-data train-data))
      (setf test-data (convert-data test-data))
      ;; build a model and train
      (setf 2nn-model (net-build train-data num-hidden-units alpha initial-bounds max-iterations 100 nil nil))
      (setf v (first 2nn-model))
      (setf w (first (last 2nn-model)))
      ;; run forward-propagate to calculate the error
      (dolist (item test-data)
	...
	(setf aver-error (+ error aver-error)))
      ...
      ;; calculate the average error for one chunk
      (setf aver-error (/ aver-error (length test-data)))
      (format t "The average error of fold ~1D: ~9F ~%" (+ i 1) aver-error)
      ;; sum to the total error average
      (setf total-error-aver (+ total-error-aver aver-error)))
    (setf total-error-aver (/ total-error-aver k))
    (format t "The total average error of k-fold validation: ~6F" total-error-aver)
    total-error-aver))
...
#|
                         ...
                         HOMEWORK 2's REPORT - CS580
                             Hoang Dung Bui
                               G01301478

In this homework, I wrote all the required functions: sigmoid, net-error, forward-propagate, back-propagate, net-build, simple-generalization, and k-fold-validation. I also built two functions to support the main fuctions: sigmoid-a-2dlist and split.

FUNCTIONS
1. sigmoid: the exp function is used to calculate the sigmoid function.

2. net-error: the dotimes loop is used to calculate the error sum.

3. forward-propagate: In this function, the functions multiple and sigmoid-a-2dlist are used to calculate the matrices multiplication and the sigmoid of a list values.

4. back-propagate: The back-propagation is processed by matrix approach. All the node values such as  output, input and hidden nodes are converted into vector and matrices; Then the functions such as multiply, scalar-multiply, sigmoid-a-2dlist, e-multiply, add are deployed to calculated the updated matrix delta-v and delta-w.

5. net-build: In this function, the neural network model (matrices v and w) are initialized. These matrices are modified by calling the function back-propagate. The model is trained in duo loops: dotimes - dolist. In the dotimes loop, it will runs until reaching the max-interation or the errors is smaller than a threshold. Insides the dotimes, the dolist loop will run through the dataset and call the function back-propagate to modify the neural network model. Based on the modulo value, worst error and mean error can be calculated for each epoch. To show all the errors, the print-all-error is needed to set True. This function will return the neural network model. 

6. simple-generalization: This function implements two tasks: create the model and train it by calling the funtion net-build. As the training finishes, it will calculate the error based on the test-data. The train-data and test-data are splited from on original dataset. The raw materials also are pre-processed by the function convert-data into the proper form. The two dataset: *wine* and *mpg* are needed to be normalized by function scale-data before input to this function.

7. k-fold-validation: In this function, the dataset is splitted into k fold, and (k-1)/k of the dataset is used to train the model and 1/k of the dataset is used to test the model. Each fold is used one time as a testing data, and we created k models. The average error is calculated based on the error of each model of its corresponding fold used as test-data. To ease the split  the data into k-fold, I wrote the function split. 

8. sigmoid-a-2dlist: I used a dolist to put each list item to the sigmoid function to calculate the sigmoid value.

9. split: The function subseq, reduce and nthcdr are used to separate a fold data from the dataset. The data position of divided data are arbitrary.

THE RESULTS:
1. For *NAND* dataset: 
As calling (net-build (convert-data *nand*) 3 1.0 5 20000 1000 t) about 25 times, there were three times the model ran up to 20000 iteration. Sometimes, it needed only 5000 iteration to converge (the error is smaller than *a-good-minimum-error*)

2. For *XOR* dataset:
As calling (net-build (convert-data *xor*) 3 1.0 5 20000 1000 t) about 25 times, the same situtation happened with three times the model ran up to 20000 iteration. In shortest training period, it needed only 3999 iteration to converge (the error is smaller than *a-good-minimum-error*)

3. For *VOTING-RECORD* dataset.
As calling (net-build (convert-data *voting-records*) 10 1.0 2 5000 250 t), it never converged in 5000 iteration. The worst error was around 0.005 and the mean error was around 0.00021.

As increasing the hidden nodes from 15 to 22, and learning rate from 1.0 to 0.5, the average errors reduced significantly to 0.000040, but the worst error did not change much, around 0.001-0.003. The lowest worst error and average error is 0.0009/0.000021 with 20 hidden nodes  and learning rate is 0.8.

As involing the function (simple-generalization *voting-records* 22 0.8 2.0 5000), the average error on test data was around 0.023, it seemed the neural network is overfitting. We should reduce the complexity of the mode. 

As invoking the function k-fold-validation, with k=4, 20 hidden layers, learning rate 0.8, and 3000 iterations,  the total average error of all folds were about: 0.014

4. For *mpg* dataset.

To input this dataset into the neural network model, the dataset is needed to be normalized by function scale-data. Then the normalized data can be inputted into the model. 

As calling (simple-generalization (scale-data *mpg*) ...) with 9 hidden nodes, learning rate = 0.7, 5000 iterations, the average error on test data is 0.0048 

As calling (k-fold-validation (scale-data *mpg), with k=5, hidden nodes = 10, learning rate =0.8, and 3000 iterations, the total average errors of 5 folds were around 0.002.

5. For *wine* dataset:
This dataset needed to be normalized to input the function simple-generalization and k-fold-validation.

As testing on the function (simple-generalization (scale-data *wine*), with 19 hidden layers, learning rate 0.5, 5000 iteration, the error were around 0.32.

As testing on (k-fold-validation (scale-data *wine*) with 5 folds, 19 hidden layers learning rate 0.2 and 3000 iterations, the average errors is 0.04. The error is much lower than the regular validation.


THE LEARNING
1. From the testing on some dataset, the more train data we have the more accuracy the model is. For example, as the *xor* and *nand* data, the model needs much more iteration to converge.  

2. k-fold-validation provided a better result then the regular validation. It is a good choice to handle the small dataset.
