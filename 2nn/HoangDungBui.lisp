...
(defun net-build (data num-hidden-units alpha initial-bounds max-iterations modulo &optional print-all-errors (error-each-epoch t))
  (let (input-size output-size (w nil) (v nil) (vw nil) (worst-error 0) (mean-error 0) error predict data1 (j 0))
	...
	(setf vw (back-propagate item alpha v w))
	(setf v (first vw))
...
6. For *red-wine-quality* dataset
This is the dataset which I downloaded from https://archive.ics.uci.edu/ml/index.php. It consists of 11 attributes and the output is the wine quality ranging from 1 to 10. This dataset needs to be normalized by function scale-data before inputting into the function simple-generalization and  k-fold-validation.

As calling the function simple-generalization with parameters 12 the hidden layer (11 attributes), learning rate 0.5, and 3000 iterations, the error on test data was around 0.006

As using k-fold-validation with parameters: k = 6, hidden layer number 12, learning rate 0.5, 3000 iterations, the average error of 6 folds was: 0.0053, a little lower than the error from function simple-generalization.
