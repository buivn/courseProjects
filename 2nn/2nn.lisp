...
the datum as input."
  (let (o1 o2 o3)
    (setf o1 (multiply v datum))
    ;;(print o1)
    (setf o2 (sigmoid-a-2dlist o1))
    ;;(print (mapcar 'sigmoid o1))
    (setf o3 (multiply w o2))
    (sigmoid-a-2dlist o3)))
...
(defun sum-matrix-column (matrix)
  (let ((output-list nil) rl (sum 0) j)
...
returning a list consisting of new, modified V and W matrices."
  ...
  ;; same let* statement.
  (let* ((o nil) (h nil) (sig-list-h nil) (sig-list-o nil) (delta-w nil) (delta-v nil)  term1 term2 i j (w-delta-w nil) (ew nil) (label nil) (datum nil))
    ...
    ;; calculate the update term and then update for matrix w 
    (dotimes (i (length o))
      (setf term1 (- (elt label i) (elt (elt sig-list-o i) 0)))
      (setf term2 (- 1 (elt (elt sig-list-o i) 0)))
      ;; calculate the matrix delta-w 
      (dotimes (j (length h))
	(setf (elt (elt delta-w i) j) (* alpha (* term1 (* (elt label i) (* term2 (elt (elt sig-list-h j) 0))))))))

    ;;; calculate the w*delta-w
    (setf w-delta-w (sum-matrix-column (e-multiply w delta-w)))
    
    ;; setup the size of matrix delta-v 
    (setf delta-v (make-random-matrix (length v) (length (first v)) 0.01))

    ;; calculate the update term delta-v 
    (dotimes (j (length v))
      (setf term2 (- 1 (elt (elt sig-list-h j) 0)))
      ; multiply w-delta-w and (1-h)
      (setf term2 (* term2 (elt (elt w-delta-w j) 0)))      
      (dotimes (k (length datum))
	(setf (elt (elt delta-v j) k) (* term2 (elt (elt datum k) 0)))))
...
value between -(INITIAL-BOUNDS) and +(INITIAL-BOUNDS)."
  (let (input-size output-size (w nil) (v nil) i j (vw nil) (worst-error 0) (mean-error 0) error  predict)
	...
	(setf v (first vw))
	(setf w (first (rest vw))))
      ;; run forward-propagate to calculate the error
      (dolist (item data)
	(setf predict (forward-propagate (first item) v w))
	(setf error (abs (- predict (first (first (rest item))))))
	(if (and print-all-errors)
	    (print error))
	(if (< worst-error error)
	    (setf worst-error error))
	(setf mean-error (+ error mean-error)))
      (setf mean-error (/ mean-error (length data)))
      (format t "The worst error: ~6F" worst-error)
      (format t "The mean error: ~6F" mean-error)
      ;; terminate the dotimes list
...
and use a modulo of MAX-ITERATIONS."
  (let ((ave-error 0) 2nn-model processed-data train-data test-data v w error average-error predict)
    ...
    (setf 2nn-model (net-build train-data num-hidden-units alpha initial-bounds max-iterations max-iternations nil))
	...
	;;  (print error))
      ;;(if (< worst-error error)
	;;  (setf worst-error error))
      (setf average-error (+ error average-error)))
    (setf average-error (/ average-error (length test-data)))
    (format t "The average error: ~6F" average-error)
    average-error))
